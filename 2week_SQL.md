...
SELECT : 컬럼선택, AS로 별칭 지정가능
WHERE : FROM의 테이블 데이터를 필터링(조건설정)
FROM : 데이터 볼 테이블명, AS로 별칭 지정가능
작성순서는 SELECT -> FROM -> WHERE
쿼리엔진의 실행순서는 FROM -> WHERE -> SELECT
...
그룹바이 : GROUP BY + (AVG, MAX)
고유값  : DISTINCT
조건설정 : WHERE/HAVING(둘의 차이는 집계 전/후)
정렬 : ORDER BY
출력개수 제한 : LIMIT
+a 일반쿼리문 -> ORDER BY -> LIMIT 순으로 작성
...
~~~
잘못된부분 : 집계 후 WHERE을 사용했음 집계 후엔 HAVING을 사용해야함
